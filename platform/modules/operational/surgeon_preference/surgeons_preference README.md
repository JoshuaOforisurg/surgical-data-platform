...
Most UK theatres still rely on paper preference books or rigid EPR modules (Cerner/EPIC/CareFlow) that don’t scale across:
multiple specialities
dozens of surgeons
each surgeon having many procedures complex instrument/consumable requirements

This platform provides a modern, structured, version‑controlled alternative with two parallel pipelines:
Standalone Pipeline — Streamlit UI → Validation → Database → Kardex PDF
EPR‑Integrated Pipeline — Simulated Cerner/EPIC feed → Adapter → Database → Kardex PDF
...
Code
                   ...
                   └───────────────┬──────────────┘
                     ...
                     Transformation & Business Rules
                                   │
                                   ▼
                     PostgreSQL (Local or Azure Cloud)
                                   │
                                   ▼
...
Streamlit	localhost:8501	Azure App Service
Database	Docker Postgres	Azure PostgreSQL Flexible Server
