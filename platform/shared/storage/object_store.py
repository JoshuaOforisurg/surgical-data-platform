...
class ObjectStoreSettings(Protocol):
    """Minimum settings contract accepted by every storage provider."""
...
def normalise_metadata(metadata: dict[str, Any] | None) -> dict[str, str]:
    """Return metadata keys and values accepted by S3 and Azure Blob."""
...
class S3ObjectStoreClient:
    ...
    def put_text(self, key: str, text: str, content_type: str = "text/plain") -> str:
        return self.put_bytes(key, bytes_from_text(text), content_type)
    ...
    def latest_key(self, prefix: str, suffix: str = "") -> str | None:
...
class AzureBlobObjectStoreClient:
    ...
    def upload_file(
        ...
        with path.open("rb") as data:
            self._upload_bytes(
                key,
                data,
                content_type or content_type_for(path),
                metadata,
            )
        return self.uri(key)
    ...
    def put_bytes(
        ...
        self._upload_bytes(key, data, content_type, metadata)
        return self.uri(key)
    ...
    def download_file(self, key: str, local_path: str | Path) -> Path:
        path = Path(local_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        data = self.container_client.get_blob_client(key).download_blob().readall()
